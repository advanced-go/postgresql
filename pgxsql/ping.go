...
func Ping[E runtime.ErrorHandler](ctx context.Context) (status *runtime.Status) {
	...
	if dbClient == nil {
		return e.Handle(runtime.RequestId(ctx), pingLoc, errors.New("error on PostgreSQL ping call : dbClient is nil")).SetCode(runtime.StatusInvalidArgument)
	}
	return e.Handle(runtime.RequestId(ctx), pingLoc, dbClient.Ping(ctx))
}
